...
- プロジェクト削除はゴミ箱化（soft delete: `trashedAt = now`）であり, 紐づく Task はゴミ箱化せず `projectId` を `null` にする（カスケード NULL 固定. UI 確認やカスケード選択は行わない. 詳細は本書 §確定事項）.
- Project は `trashedReason` を持たない（削除理由は "deleted" 固定相当. Task の "completed" に当たる状態が無いため）.
...
- ルーティンの復元はカスケード復元しない（デタッチで `null` 化された配下 Task の `routineId` は元へ戻さない）.
- 検索の論理要件: ゴミ箱状態での絞り込み, 当日の生成対象判定（曜日条件）が成り立つこと. 通常一覧および当日の生成対象判定はゴミ箱状態（`trashedAt != null`）の Routine を除外する.
...
- **プロジェクトをゴミ箱に入れる際の配下 Task の扱い**: **配下 Task はゴミ箱化せず `projectId` を `null` にする（カスケード NULL 固定）**. アプリケーション層が同一トランザクション境界で Project のゴミ箱化（`trashedAt = now`）と配下 Task の `projectId` NULL 化を行う. UI でのユーザー確認やカスケード選択は無い. Project の復元はカスケード復元しない（`null` 化された `projectId` は元へ戻さない）. API 設計の詳細は feature spec（project-soft-delete）を参照する.
