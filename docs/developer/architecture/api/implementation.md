...
> 抽象アーキテクチャ上の「API」コンポーネント（サーバ - クライアント間の通信契約）を, どのプロトコル・フォーマット・認証方式・仕様表現で実装するかを示す.
> 抽象側の方針（リソース概観・認証・バージョニング）は重複して書かない. 抽象側を参照すること.

## 1. 対応する抽象概念

- API 概要（抽象）: [`overview.md`](overview.md).
- 抽象アーキテクチャ概要: [`../overview.md`](../overview.md) §9「API の位置づけ」, §7.4「認証・通信」.
- モジュール境界: [`../module-boundaries.md`](../module-boundaries.md) §7「境界をまたぐ際の約束」（通信スキーマ）.
...
| シリアライズ | UTF-8 JSON | （実装段階で別案を採らない） |
...
## 3. 応答区分と HTTP ステータスコードの対応

抽象側の応答区分（[`overview.md`](overview.md) §6）を, 実装では次の HTTP ステータスコードで表現する.
...
- **冪等性**: リセット処理エンドポイント（`/api/v1/reset`）は冪等であること（NFR-020）. 既に同じ境界時刻でリセット済みなら **no-op の 200 OK を返し, レスポンスボディに「適用済み境界時刻」を含める**（クライアントが「再実行されたか」を判別できるようにする）.
...
- 初期版はリソース・認証スキームの骨格のみ. 個別エンドポイントの request / response スキーマは feature spec の進行に合わせて追記する.
...
- ADR: [`../../adr/0006-distribution-topology.md`](../../adr/0006-distribution-topology.md), [`../../adr/0010-api-design.md`](../../adr/0010-api-design.md)
