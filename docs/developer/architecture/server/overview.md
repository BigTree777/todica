...
- 永続化機構: [`../database/overview.md`](../database/overview.md).
...
| 永続化機構 | SQLite（WAL モード, サーバプロセス内蔵） — 詳細は [`../database/overview.md`](../database/overview.md) | PostgreSQL / ファイル KVS |
...
- API 設計（プロトコル・認証・バージョニング）: [ADR-0010](../../adr/0010-api-design.md). 詳細は [`../api/overview.md`](../api/overview.md).
...
- **CORS / 許可オリジン**: `cors` ミドルウェアで Origin を検証する. 許可オリジンは環境変数 `ALLOWED_ORIGINS` (カンマ区切り) で指定し, 未指定時は default の `http://localhost:5173` (Vite dev) + `capacitor://localhost` (Android Capacitor) を使う. 許可ヘッダは `Content-Type` / `Authorization` / `Idempotency-Key` / `If-Match`. 楽観ロックは body の `version` + `If-Match` で完結し ETag は使わないため expose ヘッダは設定しない. 詳細は `server/src/app.ts:28-77`.
...
- **認証トークンの取り扱い**: パスワード平文や bcrypt ハッシュをリポジトリにコミットしない. パスワードは bcrypt ハッシュとして DB の `app_password` テーブルに 1 行だけ保持する. 初期パスワードはブラウザの InitialSetupView から設定し, サーバ起動時の env 経由 seed は行わない. ハッシュは bcrypt 内蔵の constant-time 比較で検証する. opaque token は 32 byte 乱数として `sessions` テーブルに永続化し, 期限 30 日 + ログアウト / パスワード変更で即時 revoke する.
- **OS / セキュリティ更新**: VPS / 自宅サーバいずれでも OS 更新・依存パッケージ更新は本人責任. 自動更新の方針は運用ドキュメントに分離する.
- **バックアップ**: 永続化機構のスナップショット（SQLite ファイルのコピー）を定期取得する. 手順は [`../database/overview.md`](../database/overview.md).
