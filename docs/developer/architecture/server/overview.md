...
- **CORS / 許可オリジン**: `cors` ミドルウェアで Origin を検証する. 許可オリジンは環境変数 `ALLOWED_ORIGINS` (カンマ区切り) で指定し, 未指定時は default の `http://localhost:5173` (Vite dev) + `capacitor://localhost` (Android Capacitor) を使う. 許可ヘッダは `Content-Type` / `Authorization` / `Idempotency-Key` / `If-Match`, expose ヘッダは `ETag`. 詳細は `server/src/app.ts:28-77`.
