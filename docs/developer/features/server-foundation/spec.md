...
- ゴール:
  ...
  - 環境変数（DATABASE_PATH / PORT）の仕様をドキュメント化する。
...
- FR-002: 認証は `/api/v1/login` で発行した opaque token を sessions テーブルで検証する。
...
- NFR-002（アクセス制御）: `/api/v1/login` で発行した opaque token を Bearer トークンとして使用する。初回設定とログイン方法を運用ドキュメントに記載する。
...
```
シナリオ: DB が空の状態でもサーバを起動できる
  Given  DATABASE_PATH と PORT は設定済み
  And    app_password テーブルに行が存在しない
  When   npm start を実行する
  Then   サーバは正常に起動する
  And    GET /api/v1/auth-state が初期設定未完了を返す

シナリオ: 正常起動してリクエストを受け付けられる
  Given  DATABASE_PATH / PORT が設定されている
  When   npm start を実行する
  Then   "Todica server listening on http://localhost:<PORT>" がコンソールに出力される
  And    curl http://localhost:<PORT>/healthz が HTTP 200 を返す

シナリオ: Bearer 認証が機能する
  Given  サーバが起動し、ログイン済みである
  When   Authorization: Bearer <opaque-token> ヘッダ付きで API エンドポイントにリクエストする
  Then   HTTP 200 を返す
