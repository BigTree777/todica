...
Todica サーバの認証は bcrypt ハッシュ化したパスワードとセッショントークンによる Bearer 認証で構成されている. パスワードハッシュをサーバ設定に固定すると, 個人運用の Todica であってもパスワード変更にホスト側の操作が必要になる.

1. サーバホストに SSH で接続する.
2. サーバ設定を編集し, 新しい bcrypt ハッシュを書き込む.
3. Todica サーバを再起動する.
...
本 feature では, パスワードを DB に永続化したうえで, SettingsView 内のフォームからブラウザだけでパスワードを変更できるようにする. 初回設定は `GET /api/v1/auth-state` と未認証の `POST /api/v1/password` を使い、ブラウザから完結させる.
...
- **初回設定を DB に直接保存する**. `app_password` が空のときは初期設定画面から未認証の `POST /api/v1/password` を呼び、パスワードハッシュを 1 行 INSERT する.
...
- **FR-PWD-2: 初期設定**
  - `app_password` テーブルが空なら `GET /api/v1/auth-state` は初期設定未完了を返す.
  - 初期設定未完了時のみ、未認証の `POST /api/v1/password` で最初のパスワードを保存できる.
...
### AC-8: DB が空での初回起動は初期設定画面へ遷移する (FR-PWD-2)

```
シナリオ: DB 空のときはブラウザから初期パスワードを設定できる
  Given app_password テーブルに行が存在しない
  When  サーバを起動する
  Then  GET /api/v1/auth-state は初期設定未完了を返す
  When  初期設定画面から新しいパスワード "P0" を送信する
  Then  app_password テーブルに 1 行 INSERT される
  And   その password_hash は "P0" を bcrypt.compare で検証可能である
```

### AC-9: DB に値があれば初期設定済みとして扱う (FR-PWD-2)
...
シナリオ: 既存の DB 値を起動時に上書きしない
  ...
  And   GET /api/v1/auth-state は初期設定済みを返す
  And   POST /api/v1/login は { password: "P_DB" } で 200 を返す
```
