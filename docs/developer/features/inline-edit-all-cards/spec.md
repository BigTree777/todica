...
- **REQ-3 (`<RoutineCard>` の編集モード撤去 + 全フィールド常時編集)**
  ...
  - **BL-073 (routine-card-align-with-form) で変更**: BL-071 の 3 段構造 (header に name input + PriorityStars) からさらに 4 段構造 (`.routine-card__header` に PriorityStars 単独 / 新設 `.routine-card__title` 段に name input + visually-hidden label / `.routine-card__day-checkboxes` / `.routine-card__actions`) に再編. 本 BL の blur ロジック (空文字復元 / 同値短絡) および即時 PATCH 経路はカード側 DOM 階層 (`.routine-card__header` 内 → `.routine-card__title` 内) が変わるだけで挙動は無改修. 詳細は [`../routine-card-align-with-form/spec.md`](../routine-card-align-with-form/spec.md).
