...
- **タイムゾーン入力 UI / multi-TZ 設定**: 本 feature では扱わない。リセット時刻の TZ 解釈は [`../reset-time-rework/spec.md`](../reset-time-rework/spec.md) を正本とする。
...
リセット時刻の TZ 解釈と受け入れ基準は
[`../reset-time-rework/spec.md`](../reset-time-rework/spec.md) を参照する。

## 未決事項 / 確認待ち

- **U-001: `今日の境界時刻` の算出方法（日付またぎの扱い）**
  - 確定した算出方法は [`../reset-time-rework/spec.md`](../reset-time-rework/spec.md) を参照する。
