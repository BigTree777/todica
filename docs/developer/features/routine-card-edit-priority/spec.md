...
- **D-007 (CSS 改修の最小化 / `routine-card.css` 無改修)**:
  ...
  - **BL-070 (inline-edit-all-cards) / BL-071 (routine-card-header-layout) で関連変更**: BL-070 で「編集モード」概念が撤去され, BL-071 で `<RoutineCard>` 表示モード DOM が `.routine-card__main` 撤去 + `.routine-card__header` / `.routine-card__day-checkboxes` / `.routine-card__actions` の 3 段構造に再編された. `<PriorityStars />` は `.routine-card__header` 右側に配置され, 即時 `onDefaultPriorityChange` 経路は本 D 章の方針どおり維持されている.
  - **BL-072 (routine-form-card-header-layout) で関連変更**: 本 BL のスコープ「`<RoutineCard>` 編集モードの PriorityStars」は BL-072 では対象外で, BL-072 は `<RoutineFormCard>` (起票カード) のレイアウトのみ変更する (4 段構造に再編). 編集モードの即時 `onDefaultPriorityChange` 経路は無改修. 詳細は [`../routine-form-card-header-layout/spec.md`](../routine-form-card-header-layout/spec.md).
  - **BL-073 (routine-card-align-with-form) で関連変更**: 表示カード `<RoutineCard>` のレイアウトのみ 4 段構造 (`.routine-card__header` / `.routine-card__title` / `.routine-card__day-checkboxes` / `.routine-card__actions`) に再編. 「編集モード」概念は既に BL-070 で撤去済のため本 BL の編集モード PriorityStars は適用対象外で, BL-073 後も表示カードの即時 `onDefaultPriorityChange` 経路は無改修で維持される. 詳細は [`../routine-card-align-with-form/spec.md`](../routine-card-align-with-form/spec.md).
