...
- [ ] `web/src/auth/authed-fetch.test.ts` を新規作成し, ファイル先頭にコメントブロックで
      受け入れ基準の出典 (`docs/developer/features/auth-storage-tests/spec.md` AC-1〜AC-7) を明記する.
- [ ] `beforeEach` / `afterEach` に共通セットアップ (mock storage 注入, `global.fetch` mock,
      `setAuthStorage(null)` でのクリーンアップ) を実装する.
- [ ] AC-1: `authedFetch("/api/v1/projects", { method: "POST", body: "x" })` が `fetch` を
      `"/api/v1/projects"` + 第 2 引数に `Authorization: Bearer tkn-1` を含む `Headers` 付きで
      呼ぶことを assert する.
- [ ] AC-2: 200 Response で `clearToken` 未呼出 / event 未 dispatch を assert する.
- [ ] AC-7: 呼出側が `Authorization` を既に渡している場合に上書きしないことを assert する.
- [ ] `npx vitest run web/src/auth/authed-fetch.test.ts` で green を確認.

### Step 2: authedFetch の 401 hand-off / 非 401 素通し

- [ ] AC-3: 401 Response で `currentStorage.clearToken` が 1 回呼ばれ,
...
- [ ] AC-4: 500 Response で `clearToken` 未呼出 / event 未 dispatch を assert する.
- [ ] `npx vitest run web/src/auth/authed-fetch.test.ts` で green を確認.

### Step 3: authedFetch の storage 未設定 / getToken=null

- [ ] AC-5: `setAuthStorage(null)` 状態で 401 Response を受けたとき, `fetch` への
      Authorization ヘッダが付かないこと / event は dispatch されることを assert する.
- [ ] AC-6: `getToken().mockResolvedValue(null)` のとき Authorization 非付与であること,
      200 Response が透過することを assert する.
- [ ] `npx vitest run web/src/auth/authed-fetch.test.ts` で green を確認.

### Step 4: CapacitorAuthStorage の getToken

- [ ] `web/src/auth/capacitor-auth-storage.test.ts` を新規作成し, ファイル先頭で
      `vi.mock("@capacitor/preferences", () => ({ Preferences: { get: vi.fn(), set: vi.fn(), remove: vi.fn() } }))` を宣言する.
- [ ] `beforeEach` で `vi.clearAllMocks()` を呼ぶ.
- [ ] AC-8: `Preferences.get.mockResolvedValue({ value: "tkn-stored" })` のとき,
      `new CapacitorAuthStorage().getToken()` が `"tkn-stored"` を返し, `Preferences.get` が
      `{ key: "authToken" }` で 1 回呼ばれることを assert する.
- [ ] AC-9: `Preferences.get.mockResolvedValue({ value: null })` のとき, 返り値が `null` であることを assert する.
- [ ] `npx vitest run web/src/auth/capacitor-auth-storage.test.ts` で green を確認.

### Step 5: CapacitorAuthStorage の setToken / clearToken

- [ ] AC-10: `CapacitorAuthStorage.setToken("tkn-new")` で `Preferences.set` が
      `{ key: "authToken", value: "tkn-new" }` で 1 回呼ばれ, subscribe した listener が
      `"tkn-new"` で呼ばれることを assert する.
- [ ] AC-11: `CapacitorAuthStorage.clearToken()` で `Preferences.remove` が `{ key: "authToken" }` で
      1 回呼ばれ, subscribe した listener が `null` で呼ばれることを assert する.
- [ ] `npx vitest run web/src/auth/capacitor-auth-storage.test.ts` で green を確認.

### Step 6: CapacitorAuthStorage の round-trip

- [ ] AC-12: module-scoped な `let storedValue: string | null = null;` を `Preferences.set` mock 内で
...
- [ ] `npx vitest run` をリポジトリルートから実行し, 既存 1666 件 + 新規分すべて green を確認 (AC-13).
- [ ] `npm run typecheck` (web ワークスペース) が 0 エラーであることを確認.
- [ ] `npm run lint` (web ワークスペース) が 0 エラーであることを確認.

## テスト

- [x] 単体テスト (Step 1〜6 で記述)
- [ ] 結合 / E2E テスト (本 feature のスコープ外 / 既存 `app-login-production-path.test.tsx` を活かす)

## ドキュメント

- [ ] 関連ドキュメント更新は不要 (テスト追加のみ. API / schema / user ガイドに影響なし).

## 仕上げ

- [ ] 受け入れ基準 (spec.md AC-1〜AC-13) を全て満たすことを確認.
- [ ] `auditor` にレビュー依頼 (production 無改修 / 既存テスト無改修 / カバレッジ追加の確認).
- [ ] PR タイトル例: `test(auth-storage-tests): BL-078 authedFetch / CapacitorAuthStorage の単体テスト追加`.
