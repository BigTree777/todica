...
- [x] **失敗テスト**: `web/src/auth/auth-storage.test.ts` 新規追加 (AC-2 / AC-4 / AC-5 の基盤).
  ...
  - [ ] Capacitor 実装: モック `@capacitor/preferences` で `Preferences.set/get/remove` が呼ばれる. (Capacitor 実装は提供. ユニットテストでの動的 import モックは未追加.)
- [ ] **失敗テスト**: `web/src/auth/authed-fetch.test.ts` 新規追加. (実装は提供したが test-designer が単体テストを書いていない. 401 hand off は `web/__tests__/app-login.test.tsx` で間接的に検証.)
- [x] **実装**:
  ...
  - [ ] `web/src/repositories/task-repository.ts` 等 5 本: コンストラクタから `authToken` 引数を撤去し `authed-fetch` を使う. (本 BL では HTTP Repository の constructor 互換性を維持. token は `main.tsx` の `App` state 経由で都度差し替え. 既存 repository tests への影響を最小化.)
- [x] 上記単体テストが green になることを確認.
...
- [ ] **失敗テスト**: `e2e/pwa-prod.spec.ts` または新規 `e2e/sw-api-bypass.spec.ts` に以下のテストを追加. (E2E 追加は本 BL では未実施. vitest 範囲外.)
- [x] **実装**:
...
- [ ] 既存 PWA E2E (`e2e/pwa-prod.spec.ts`) が green を維持することを確認. (E2E は本タスクの範囲外.)
...
- [ ] **E2E** `e2e/login.spec.ts` 新規追加. (vitest 範囲外のため未対応.)
