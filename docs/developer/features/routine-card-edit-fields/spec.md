...
- **D-008 (CSS 改修の最小化)**:
  ...
  - **BL-073 (routine-card-align-with-form) で関連変更**: 表示カード `<RoutineCard>` のレイアウトのみ 4 段構造 (`.routine-card__header` / `.routine-card__title` / `.routine-card__day-checkboxes` / `.routine-card__actions`) に再編. 曜日 checkbox / PriorityStars の即時 `onDaysOfWeekChange` / `onDefaultPriorityChange` 経路は本 D-008 の方針どおり無改修で BL-073 後も維持される. 詳細は [`../routine-card-align-with-form/spec.md`](../routine-card-align-with-form/spec.md).
