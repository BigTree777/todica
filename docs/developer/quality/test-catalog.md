...
| `web/__tests__/settings-view.test.tsx` | 13 | SettingsView（境界時刻・サーバ設定・モード切替・ログアウトボタン） うち `describe("SettingsView サーバ接続設定セクション (BL-019 AC-AND-005)")` の 3 件は BL-075 で削除予定 | BL-009, 019〜020, 074, 075 |
| `web/__tests__/settings-view-dead-path-removed.test.tsx` | 4 | SettingsView から旧 `serverUrl` / `authToken` / `onSaveServer` 由来の dead path (`<section aria-label="サーバ接続設定">` / id=settings-server-url / id=settings-auth-token / 「変更を保存」ボタン) が DOM 上から消えていることを検証 | BL-075 |
