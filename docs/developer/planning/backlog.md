...
| BL-027 | クロスレイヤ整合性 E2E テスト群（Playwright 側で書ける範囲） | P2 | Doing | 単体・統合テストでは原理的に検証不能な領域のうち, Playwright 標準機能のみで書けるもの. 対象: ページリロード後の focus / 完了数 / 設定の復元, 同 Idempotency-Key 重複送信の冪等応答, 2 タブ同時編集 (複数 BrowserContext) での If-Match 412 → ConflictDialog 表示 (BL-018), オフライン中の追加 → IndexedDB キュー保存 → オンライン復帰で自動 flush (BL-018), 401 / ネットワークエラー時の UI 反応. server 側に新規コード追加が必要な項目 (server 再起動でデータ保持 / 境界時刻またぎでのリセット連動) は BL-030 に切り出し |
