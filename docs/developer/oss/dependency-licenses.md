...
`server` workspace に固有の devDependencies は無く、ルートの devDependencies
（TypeScript・vitest 等）を共有する。

## domain workspace（dependencies）

`@todica/domain` はモノレポ内部の純粋ロジックパッケージで、**外部ランタイム依存・固有の
devDependencies を持たない**（型・テスト等はルートの devDependencies を共有する）。
配布物にはビルド済みの内部コードのみが含まれ、追加の第三者ライセンスは発生しない。
