...
| 変数名 | 必須 | デフォルト | 説明 |
|---|---|---|---|
| `APP_PASSWORD_HASH` | はい | - | アプリログインパスワードの bcrypt ハッシュ。未設定だと起動失敗（`process.exit(1)`） |
...
| 症状 | 原因 | 対処 |
|---|---|---|
| `APP_PASSWORD_HASH environment variable is required` で exit | `.env` を読めていない / `APP_PASSWORD_HASH` が空 | `journalctl -u todica` でログ確認、`EnvironmentFile=` のパスと `.env` の権限を確認。bcrypt ハッシュは `node -e "console.log(require('bcrypt').hashSync('your-password', 12))"` で生成 |
