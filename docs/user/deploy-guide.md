...
| `TZ` | 推奨 | OS の `/etc/localtime` から推定 | サーバプロセスの IANA タイムゾーン。日次リセット (`dayBoundaryTime` HH:MM) の解釈に使う。`/etc/localtime` がシンボリックリンクでない環境では Node が `UTC` にフォールバックし、JST 設定でも UTC 基準でリセット判定する。JST 運用なら `Asia/Tokyo` を明示する |
...
## 7. 初期パスワードを設定する（最初のアクセス）

サーバを起動して nginx を reload した直後、`https://todica.example.com/` をブラウザで開くと **初期パスワード設定画面** が表示される。

1. 新しいパスワードを 1 つ目の入力欄に入れる
2. 確認のため同じパスワードをもう一度 2 つ目の入力欄に入れる
3. 「設定する」を押す

成功すると bcrypt ハッシュが DB の `app_password` テーブルに保存され、そのままセッショントークンが発行されて自動ログインされる。今日のタスク一覧（最初は空）が表示される。

以降、ブラウザでアクセスすると LoginView が表示され、設定したパスワードでログインする。

> **⚠️ セキュリティ上の注意**: 「サーバを起動してから最初に公開 URL に到達した人がパスワードを設定できる」性質がある。デプロイ直後はただちに運用者本人がブラウザで開いて初期設定を完了し、他人が先に到達するリスクをゼロにすること。
>
> 万一他人が先にパスワードを設定してしまった場合は、サーバに SSH で入って `sqlite3 /var/lib/todica/todica.db "DELETE FROM app_password;"` を実行し、ブラウザを再読み込みして自分で初期設定をやり直す。
...
| 設定したリセット時刻 (`dayBoundaryTime`) を過ぎてもリセットが走らない（未完了タスクが翌日に繰越されない / 完了数が 0 に戻らない） | Node が解決した TZ が `UTC` のままで、`HH:MM` を UTC として解釈している。OS の `date` が JST でも、`/etc/localtime` がシンボリックリンクでない / tzdata 不在の環境では Node が `Intl.DateTimeFormat().resolvedOptions().timeZone` の自動推定に失敗する | `journalctl -u todica \| grep "resolved timezone"` で Node が解決した TZ を確認する。`UTC` 等になっていれば `.env` の `TZ=Asia/Tokyo`（または systemd unit の `Environment=TZ=Asia/Tokyo`）を確認し、`sudo systemctl restart todica` で反映 |
