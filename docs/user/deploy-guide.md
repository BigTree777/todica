...
- インターネット越し公開を行う場合、認証なしモードは禁止（`APP_PASSWORD_HASH` を必ず設定する）
...
| 変数名 | 必須 | デフォルト | 説明 |
|---|---|---|---|
| `APP_PASSWORD_HASH` | 初回 seed 時 | - | アプリログインパスワードの bcrypt ハッシュ。DB が空の初回起動時だけ seed に使う。起動後は DB が真の source となり、SettingsView から変更可能。DB が空かつ未設定の場合のみ起動失敗する |
...
`APP_PASSWORD_HASH` は bcrypt cost factor 12 で生成する：

```bash
node -e "console.log(require('bcrypt').hashSync('your-password', 12))"
```

生成したハッシュ全体（`$2b$12$...`）を `.env` の `APP_PASSWORD_HASH` に貼り付ける。
平文パスワードは `.env` に置かない。ユーザはアプリの LoginView で平文パスワードを入力し、
サーバ側で `bcrypt.compare` により照合される。初回 seed 後のパスワードは SettingsView から変更でき、
以降の認証では DB に保存されたハッシュが使われる。
...
| 症状 | 原因 | 対処 |
|---|---|---|
| `APP_PASSWORD_HASH environment variable is required` で exit | DB が空で、`.env` を読めていないか `APP_PASSWORD_HASH` が空 | `journalctl -u todica` でログ確認、`EnvironmentFile=` のパスと `.env` の権限を確認。bcrypt ハッシュは `node -e "console.log(require('bcrypt').hashSync('your-password', 12))"` で生成 |
