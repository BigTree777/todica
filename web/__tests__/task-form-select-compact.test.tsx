...
describe("起票カードのプロジェクト <select> の box サイズを縮小 (BL-066 / task-form-select-compact)", () => {
   ...
   */
  describe("AC-5: TaskCard 表示側 chip は本 BL ルールの影響を受けない (G-4 / NFR-CHIP-PRESERVE)", () => {
    it(".project-chip の computed font-size が '14px' または 'var(--font-size-small)' である (BL-063 D-003 由来)", () => {
      ...
      const chip = container.querySelector(".project-chip") as HTMLElement | null;
      expect(chip, ".project-chip が見つからない").not.toBeNull();
      if (!chip) return;
      const style = getComputedStyle(chip);
      // BL-063 D-003 で .task-card__header .project-chip { font-size: var(--font-size-small) }
      // が定義済み. jsdom は var() を解決しないため "var(--font-size-small)" 文字列で返ることがある.
      expect(
        style.fontSize === "14px" || style.fontSize === "var(--font-size-small)",
        `.project-chip の font-size が '14px' でも 'var(--font-size-small)' でもない (実際: "${style.fontSize}")`,
      ).toBe(true);
    });

    it("TaskCard 表示側 DOM に <select> が 0 個である (= 本 BL ルールがマッチしようがない)", () => {
      const { container } = render(
        <TaskCard
          task={makeTask()}
          project={makeProject({ name: PROJECT_NAME_P1 })}
      ...
      expect(
        selects.length,
        `TaskCard 表示側に <select> が ${selects.length} 個存在する (構造的不変性 D-001 違反)`,
      ).toBe(0);
    });
  });
   ...
   */
  describe("AC-7: JSX の構造的不変性 sample (REQ-4 / G-3)", () => {
    it("task-card.tsx に '<select' 文字列が含まれない (TaskCard 表示側に select 無し)", () => {
