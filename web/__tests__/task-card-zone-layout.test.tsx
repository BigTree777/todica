...
function renderWithQueryClient(ui: ReactNode): ReturnType<typeof render> {
  ...
  // 本ファイルは一覧カードのゾーンレイアウトのみ検証するので `?create=1` は付けない.
  return render(
    <MemoryRouter
...
describe("タスクカードの 3 段ゾーンレイアウト (BL-057, BL-059 で .task-card 系に移譲)", () => {
  ...
  // AC-9 (BL-059 追従): .project-chip が .task-card__header 内に配置されている
  // ----------------------------------------------------------
  describe("AC-9 (BL-108 追従): プロジェクト変更 `<select>` が .task-card__header 内に配置されている", () => {
    it("today-view の project 割り当て済みカードの header 内に <select> が存在し title / actions には無い", async () => {
      ...
      // BL-108 追従: chip span は <select> に置換された.
      const selectInHeader = header?.querySelector("select") as HTMLSelectElement | null;
      expect(selectInHeader, "header 段に <select> が存在しない").not.toBeNull();
      expect(selectInHeader?.value, "<select>.value が project id でない").toBe(PROJECT_ID_P1);
      // selected option 内に project name が表示される.
      const selectedOption = selectInHeader?.querySelector(
        `option[value="${PROJECT_ID_P1}"]`,
      ) as HTMLOptionElement | null;
      expect(selectedOption?.textContent ?? "").toContain(PROJECT_NAME_P1);

      expect(title?.querySelector("select") ?? null).toBeNull();
      expect(actions?.querySelector("select") ?? null).toBeNull();
    });
  });
  ...
  // ----------------------------------------------------------
  describe("AC-10 (BL-108 追従): プロジェクト未設定タスクでも 3 段子要素が存在し <select> が居る", () => {
    it("projectId === null のタスクカードでも header 段に <select value=''> が居る", async () => {
      const focusedTask = makeTask({
        id: "task-focus",
        name: "フォーカスタスク",
        projectId: null,
      ...
      // BL-108: <select> は常に存在する. projectId === null では value="" になる.
      ...
      expect(select?.value, "未割当タスクの <select>.value が '' でない").toBe("");
      // 旧 .project-chip span は撤去済み.
