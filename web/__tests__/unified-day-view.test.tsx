 ...
 *   AC-5 : 各 `<form>` に day-view__form, 旧 tomorrow-view__form クラス無.
 *   AC-6 : 各 `<ul>` に day-view__list, 各 `<li>` に day-view__card. 旧クラス無.
 *   AC-7 : `web/src/ui/day-view/day-view.css` が存在し `.day-view__card` 等を含む.
...
describe("BL-051 / unified-day-view: AC-3 (「現在のタスク」セクションの位置)", () => {
  it("focusedTask 有り の /today で 子要素順序が [header, section(現在のタスク), form, ul] で section に day-view__card と day-view__card--focus 両方を含む", async () => {
    ...
    // 2 番目 = section[aria-label="現在のタスク"] かつ class に day-view__card / --focus.
    const second = children[1]!;
    expect(second.tagName.toLowerCase()).toBe("section");
    expect(second.getAttribute("aria-label")).toBe("現在のタスク");
    expect(second.classList.contains("day-view__card")).toBe(true);
    expect(second.classList.contains("day-view__card--focus")).toBe(true);
...
describe("BL-051 / unified-day-view: AC-5 (起票フォームに day-view__form)", () => {
  it("/today の <form aria-label='タスク起票フォーム'> の class に day-view__form を含み tomorrow-view__form を含まない", async () => {
    const repo = makeMockTaskRepository([
      makeTask({ id: "t1", name: "x", dueDate: "today", version: 1 }),
    ]);
    ...
    expect(form.classList.contains("day-view__form")).toBe(true);
    expect(form.classList.contains("tomorrow-view__form")).toBe(false);
  });

  it("/tomorrow の <form aria-label='明日のタスク起票フォーム'> の class に day-view__form を含み tomorrow-view__form を含まない", async () => {
    const repo = makeMockTaskRepository([
      makeTask({ id: "t1", name: "x", dueDate: "tomorrow", version: 1 }),
    ]);
    renderWithQueryClient(
...
describe("BL-051 / unified-day-view: AC-6 (タスク一覧 ul / li の共通クラス)", () => {
  it("/today の <ul aria-label='タスク一覧'> に day-view__list, 各 <li> に day-view__card を含む", async () => {
    ...
    for (const li of items) {
      expect(li.classList.contains("day-view__card")).toBe(true);
      // 旧 today 用クラスは付かない.
      expect(li.classList.contains("tomorrow-view__item")).toBe(false);
    }
  });

  it("/tomorrow の <ul aria-label='明日のタスク一覧'> に day-view__list, 各 <li> に day-view__card を含む (旧 tomorrow-view__list/__item は付かない)", async () => {
    const repo = makeMockTaskRepository([
      makeTask({
        id: "task-1",
        name: "明日タスク",
...
describe("BL-051 / unified-day-view: AC-7 (共通 CSS ファイル / 参照)", () => {
  ...
  it("day-view.css が REQ-7 の最小クラスセレクタ (.day-view / .day-view__header / .day-view__form / .day-view__list / .day-view__card) を含む", () => {
    expect(existsSync(dayViewCssPath)).toBe(true);
    const content = readFileSync(dayViewCssPath, "utf8");
    expect(content).toMatch(/\.day-view\b/);
    expect(content).toMatch(/\.day-view__header\b/);
    expect(content).toMatch(/\.day-view__form\b/);
    expect(content).toMatch(/\.day-view__list\b/);
    expect(content).toMatch(/\.day-view__card\b/);
  });
