...
// BL-065 (project-toggle-removal): ProjectToggle は撤去された.
// 旧 AC-4 (ProjectToggle button が .project-chip className を持つ) は describe ごと撤去済み.
...
// BL-065 (project-toggle-removal): project-toggle/ ディレクトリは撤去された.
// 旧 AC-9 (project-toggle.css に .project-chip が混入していない) は describe ごと撤去済み.
...
describe("プロジェクト名 chip 共通スタイル + タスクカード表示追加 (BL-056 / project-chip)", () => {
   ...
   */
  describe("AC-2 (BL-108 追従): today-view にプロジェクト割り当て済みタスクで `<select>` が表示される (DOM)", () => {
    it("タスク一覧内に <select> の selected option としてプロジェクト名が描画される", async () => {
      ...
      // BL-070 追従: <span>{name}</span> は input に置換されたため findByDisplayValue で待つ.
      await screen.findByDisplayValue("タスクA");

      // BL-108: タスク一覧 (<ul aria-label="タスク一覧">) スコープ内の <select> を観察.
      const list = screen.getByRole("list", { name: "タスク一覧" });
      const selects = list.querySelectorAll("select");
      expect(
        selects.length,
        "タスク一覧スコープ内に <select> が描画されていない",
      ).toBeGreaterThanOrEqual(1);

      // 少なくとも 1 つの <select> で selected option がプロジェクト名を含む.
      const selectedTexts = Array.from(selects).map((sel) => {
      ...
      expect(
        selectedTexts.some((t) => t.includes(PROJECT_NAME_P1)),
        `<select> の option にプロジェクト名 "${PROJECT_NAME_P1}" を含むものが無い (実際: ${JSON.stringify(selectedTexts)})`,
      ).toBe(true);
    });
  });
   ...
   *    かつ そのうち少なくとも 1 つのテキストが該当プロジェクトの name と一致する
   */
  describe("AC-3 (BL-108 追従): tomorrow-view にプロジェクト割り当て済みタスクで `<select>` が表示される (DOM)", () => {
    it("明日のタスク一覧内に <select> の selected option としてプロジェクト名が描画される", async () => {
      const project = makeProject({ id: PROJECT_ID_P1, name: PROJECT_NAME_P1 });
      const taskWithProject = makeTask({
      ...
      // BL-070 追従: <span>{name}</span> は input に置換されたため findByDisplayValue で待つ.
      await screen.findByDisplayValue("明日タスクA");

      // BL-108: 明日のタスク一覧スコープ内の <select> を観察.
      const list = screen.getByRole("list", { name: "明日のタスク一覧" });
      const selects = list.querySelectorAll("select");
      expect(
        selects.length,
        "明日のタスク一覧スコープ内に <select> が描画されていない",
      ).toBeGreaterThanOrEqual(1);
  ...
  // ----------------------------------------------------------
  // AC-4 (BL-065 で撤去): 旧 AC-4 「ProjectToggle の button が .project-chip className を持つ」
  //   は ProjectToggle 撤去 (BL-065 / project-toggle-removal) により describe ごと削除された.
  //   TaskCard 表示側 chip (<span className="project-chip">) の検証は AC-2 / AC-3 / AC-5 で
  //   引き続き網羅される.
  // ----------------------------------------------------------
  ...
  describe("AC-5: プロジェクト未設定タスクでは chip 自体を描画しない (D-002)", () => {
    it("today-view: projectId === null のタスクカードに .project-chip 要素が存在しない", async () => {
      ...
      // BL-070 追従: name は input value に入るため findByDisplayValue で取得.
    ...
    it("tomorrow-view: projectId === null のタスクカードに .project-chip 要素が存在しない", async () => {
      ...
      // BL-070 追従: name は input value に入るため findByDisplayValue で取得.
      const taskNameNode = await screen.findByDisplayValue("明日の未分類タスク");
      const card = taskNameNode.closest("li");
  ...
  describe("AC-7: 他セレクタへの本 BL 追記が無い (REQ-6)", () => {
    ...
    it.each(
      STRUCTURE_ONLY_SELECTORS,
    )("%s ルール本文に本 BL の project-chip 関連キーワードが含まれない", (selector) => {
      const css = readFileSync(dayViewCssPath, "utf-8");
  ...
  // ----------------------------------------------------------
  // AC-9 (BL-065 で撤去): 旧 AC-9 「project-toggle.css の不変性」は
  //   ProjectToggle 撤去 (BL-065 / project-toggle-removal) により describe ごと削除された.
  //   project-toggle.css 自体が存在しなくなるため, 不変性 assert は意味を失う.
  // ----------------------------------------------------------
