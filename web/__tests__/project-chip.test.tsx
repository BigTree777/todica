...
describe("プロジェクト名 chip 共通スタイル + タスクカード表示追加 (BL-056 / project-chip)", () => {
  ...
  describe("AC-7: 他セレクタへの本 BL 追記が無い (REQ-6)", () => {
    ...
    it(".day-view__card ルール本文は BL-052 確定値のままで, 本 BL で書き換えられていない", () => {
      ...
      // 引き上げられた (chip の角丸と同調). 本 BL (BL-056) の関心ではないが追従する.
      expect(bodyText).toMatch(/border-radius\s*:\s*var\(--radius-lg\)/);
      expect(bodyText).toMatch(/(?:^|;|\n)\s*padding\s*:\s*var\(--space-md\)/);
      // 本 BL の追記キーワードが混入していないこと.
      expect(bodyText).not.toMatch(/project-chip/);
    });

    it(".day-view__card--focus ルール本文は BL-052 確定値のままで, 本 BL で書き換えられていない", () => {
      const css = readFileSync(dayViewCssPath, "utf-8");
