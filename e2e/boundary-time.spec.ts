...
async function listTasks(
  ...
  const res = await request.get(`${API_BASE}/api/v1/tasks?trashed=false`, { headers: AUTH_HEADER });
  expect(res.status()).toBe(200);
...
async function getToday(request: import("@playwright/test").APIRequestContext): Promise<{
...
test("境界時刻またぎで dueDate=tomorrow のタスクが today に繰り越される", async ({ request }) => {
  ...
  const beforeTasks = await listTasks(request);
  expect(beforeTasks.find((t) => t.id === taskId)?.dueDate).toBe("tomorrow");

  // 24 時間進めて /today を叩く → reset 発火 → tomorrow が today に繰り越される.
  await advanceClock(request, ONE_DAY_MS);
  await getToday(request);

  const afterTasks = await listTasks(request);
...
test("境界時刻またぎで指定曜日のルーティンタスクが自動生成される", async ({ request }) => {
  ...
  const tasksAfter = await listTasks(request);
