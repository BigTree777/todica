...
test("Service Worker 登録スクリプトが配信され必要な workbox 機能が含まれている", async ({
  ...
  // 中身に precache / NavigationRoute / 書込キュー sync が含まれていれば
  // SW 設計と vite-plugin-pwa 統合が壊れていない.
  // (BL-074: `/api/*` は SW から完全除外したため runtime caching 戦略の
  //  StaleWhileRevalidate は使わない. アサーション対象外.)
