...
test("リロード後も明示的に設定したフォーカス対象が復元される (BL-043 で UI 再導入)", async ({
  ...
  const authHeader = await getApiAuthHeader(request, apiBase);
...
test("リロード後も保存したリセット時刻設定が復元される", async ({ page }) => {
  await page.goto("/settings");
  // BL-091 / BL-094 で SettingsView は input.value のみで現在値を表現する形に変わった.
  // 旧「設定値」span は撤去済. ラベルは「リセット時刻」, submit ボタンは「変更」.
  const timeInput = page.getByLabel("リセット時刻");

  // 既存値と異なる時刻を入れる.
  const currentValue = await timeInput.inputValue();
  const newValue = currentValue === "23:45" ? "22:15" : "23:45";

  await timeInput.fill(newValue);
  // 「設定フォーム」 form 内の「変更」 button. パスワード変更 section にも同名 button がある.
  await page.getByLabel("設定フォーム").getByRole("button", { name: "変更" }).click();
  await expect(timeInput).toHaveValue(newValue);
